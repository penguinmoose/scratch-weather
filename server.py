...
login = scratchconnect.ScratchConnect('<YOUR USERNAME>', '<YOUR PASSWORD>')

project = login.connect_project(project_id=<YOUR PROJECT ID>, access_unshared=True)
...
def weather(city):
    ...
    if (soup.select('#wob_loc')): # if there is a report avalible
        ...
        print('Weather: temp: ' + str(temp) + 'degrees F description: ' + info)
